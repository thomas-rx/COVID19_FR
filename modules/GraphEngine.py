...
def make_world_graph():
	...
	worldometers_api = requests.get("https://coronavirus-19-api.herokuapp.com/countries") 
	...
	for i in range(x, x + int(get_config('GraphConfig', 'countryView'))):
		if not worldometers_data[i]['country'] == 'France':
			try:
				total_cases.update({str(worldometers_data[i]['country']): int(worldometers_data[i]['cases'])})
			except:
				total_cases.update({str(worldometers_data[i]['country']): 0})

			try:
				dead_cases.update({str(worldometers_data[i]['country']): int(worldometers_data[i]['deaths'])})
			except:
				dead_cases.update({str(worldometers_data[i]['country']): 0})

			try:
				recovered_cases.update({str(worldometers_data[i]['country']): int(worldometers_data[i]['recovered'])})
			except:
				recovered_cases.update({str(worldometers_data[i]['country']): 0})
	...
	total_cases = {k: v for k, v in sorted(total_cases.items(), key=lambda item: item[1], reverse=True)}
	countries = list(total_cases.keys())

	for a in range(0 , int(get_config('GraphConfig', 'countryView'))):
		countries = [sub.replace(countries[a], get_config('TraductionGraph', countries[a])) for sub in countries] 
	...
	fig, ax = plt.subplots()
	ax_total = ax.bar(x + width/2, total_cases_formated, width, label=u'Population touchée', color = 'indianred')
	ax_recovered = ax.bar(x - width/2, total_recovered_formated, width, label=u'Population guérie', color = 'yellowgreen')
	ax_deaths = ax.bar(x - width/2, total_deaths_formated, width, label=u'Population décédée', color = 'dimgray')

	ax.set_xticks(x)
	ax.set_xticklabels(graph_labels, rotation=35, size='small', backgroundcolor='dimgray', color='white')
	ax.legend()
